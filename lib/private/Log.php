...
class Log implements ILogger {
	...
	private function processException($exception) {
		...
		// set to true to test with json stack
		$jsonStack = false;
		// set to true to format the trace string into an array
		$jsonStringStack = true;

		if ($jsonStack) {
			$traces = [];

			foreach ($exception->getTrace() as $traceEntry) {
				if (in_array($traceEntry['function'], $this->methodsWithSensitiveParameters)) {
					$traceEntry['args'] = ['*** sensitive parameters replaced ***'];
				}
				$traces[] = $traceEntry;
			}
		} else {
